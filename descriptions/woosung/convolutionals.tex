...
dilation=1, groups=1)}{
  ...
  \begin{itemizec}
    ...
    \item $h_{filter} + 2 \x padding[0] - dilation[0] \x (h_{filter} - 1) - 1
    \geq 0$
    \item $w_{filter} + 2 \x padding[1] - dilation[1] \x (w_{filter} - 1) - 1
    \geq 0$
...
\prepostc{torch.nn.Conv1d(in, out, kernel, stride=1, pad=0,
out\_pad=0, groups=1, bias=True, dilation=1, pad\_mode=`zeros')(x)}{
  \includegraphics[height=12em]{resources/conv2d.png}
}{
  \begin{itemizec}
    \item $|x| = (d_1, d_2, d_3)$\bigspace ($rank = 3$)
    \item $d_2 = in$
    \item $kernel \leq d_3 + 2 \x padding$
    \item $groups | in$ and $groups | out$
  \end{itemizec}
}{
  \begin{itemizec}
...
\subsection*{(Builtins) \texttt{torch.max\_pool2d}, \texttt{torch.nn.functional.max\_pool2d}}%{{{
\prepostc{torch.max\_pool2d(x, kernel\_size, stride=kernel\_size,
...
\prepostc{torch.nn.functional.max\_pool2d\_with\_indices(x, kernel\_size, stride=..., dilation=1,
ceil\_mode=False)}{
  \includegraphics[height=8em]{resources/maxpool2d_ri.png}
}{
  \begin{itemizec}
...
\subsection*{(Builtins) \texttt{torch.batch\_norm}, \texttt{torch.nn.functional.batch\_norm}}%{{{
\prepostc{torch.batch\_norm(x, mean, var, weight=None, bias=None, training=False,
other\_params,...)(x)}{
  %\includegraphics[height=10em]{resources/maxpool2d.png}
}{
  \begin{itemizec}
    \item $|x| = (d_1, d_2, d_3, \dots, d_k)$
    \item $k \geq 2$
    ...
    Otherwise,
    \begin{itemize}
      \item $mean$ is $None$ or $|mean| = (d_2)$ (also for $var$)
    \end{itemize}
