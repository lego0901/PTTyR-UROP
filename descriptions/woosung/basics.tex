...
\prepostc{torch.range(start, end, steps=100, out=None, ...), torch.arange(...)}{
  \includegraphics[width=\linewidth]{resources/range.png}
}{
  \begin{itemizec}
    \item $steps \geq 0$
...
\begin{align*}
  ...
  {
    \sigma \vdash \op{ft}{s, e, steps=100, out=None, ...} \Rar (steps), c
  }
\end{align*}
...
\begin{align*}
  ...
  {
    \sigma \vdash \op{randint}{n, out=None, ... } \Rar (n), \{(n \geq 0)\}
  }
\end{align*}
